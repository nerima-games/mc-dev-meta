 ...
 *
 * ---------------------------------------------------------------------------
 * mc-dev-meta depends on NOTHING, and must keep depending on nothing.
 *
 * It is the development-time workspace binder (plan.md §6 Step 0 item 2): it
 * clones the other 15 repositories into `repos/` and binds them into one pnpm
 * workspace so that `workspace:*` gives monorepo-equivalent DX before anything
 * is published. A tool that manages all 15 repositories must not be a
 * participant in their dependency graph — if it were, bootstrapping would
 * require the very packages it exists to fetch.
 *
 * Its row is therefore empty, and it appears in no other row's value set.
 *
 * This repository also holds the REFERENCE COPY of the roster, in
 * `domain/repository-roster.ts`. `test/roster.test.ts` asserts that the copy
 * below agrees with it edge for edge, so the two cannot drift inside this
 * repository. Sibling repositories still mirror it by hand.
 * ---------------------------------------------------------------------------
 */
export const REPOSITORY_POLICY = {
  thisPackage: '@nerima-games/mc-dev-meta',
