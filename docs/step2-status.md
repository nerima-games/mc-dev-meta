# plan.md §6 Step 2 の横断状況 —— 「プレビューが操作可能」なリポジトリは **0 / 15**
...
したがって 0/15 は 15 個の独立した遅れではなく、**1 本の連鎖の未着手**である。

```
publish なし → kit が実 Layer を持てない → kit の最小 E2E が書けない
            → どのリポジトリも apps/preview-*/ を作れない → Step 2 完了 0 件
```
...
| `mc-worldgen` | 地形プレビュー（**最初の遊べる成果物**） | ❌ 無 | ❌ 無 | ❌ |
| `mc-sim` | 障害物コース | ❌ 無 | ❌ 無 | ❌ |
...
- **この数字は kit の publish で一気に動く。** 0/15 は 15 個の課題ではなく 1 個の課題なので、
  進捗を測るなら「9 本のプレビュー」ではなく「kit が実 Layer を持てたか」を見ること。
